...
Add production screenshots here before publishing the repository or marketplace listing.
...
Capture screenshots from the sandbox profile:

```sh
./scripts/dev.sh --reset
```

Use `Cmd+Shift+P -> Custom UI Style: Reload` after CSS changes, then capture a clean window at a consistent size.
